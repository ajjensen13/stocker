...
func Stocks(ctx context.Context, lg gke.Logger, pool *pgxpool.Pool, jobRunId uint64, ss []finnhub.Stock, bo backoff.BackOff, bon backoff.Notify) error {
	return backoff.RetryNotify(func() (err error) {
		...
		if err != nil {
			return fmt.Errorf("failed to create transaction for loading stocks: %w", err)
		}
		...
		err = tx.Commit(ctx)
		if err != nil {
			return fmt.Errorf("failed to commit transaction for loading stocks: %w", err)
		}
...
func Candles(ctx context.Context, jobRunId uint64, pool *pgxpool.Pool, ss extract.StockCandlesWithMetadata, bo backoff.BackOff, bon backoff.Notify) error {
	return backoff.RetryNotify(func() (err error) {
		...
		tx, err := pool.Begin(ctx)
		if err != nil {
			return fmt.Errorf("failed to create transaction for loading candles: %w", err)
		}
		...
		err = tx.Commit(ctx)
		if err != nil {
			return fmt.Errorf("failed to commit transaction for loading candles: %w", err)
		}
...
func CompanyProfile(ctx context.Context, jobRunId uint64, pool *pgxpool.Pool, cp finnhub.CompanyProfile2, bo backoff.BackOff, bon backoff.Notify) error {
	return backoff.RetryNotify(func() (err error) {
		...
		tx, err := pool.Begin(ctx)
		if err != nil {
			return fmt.Errorf("failed to create transaction for loading company profiles: %w", err)
		}
		...
		err = tx.Commit(ctx)
		if err != nil {
			return fmt.Errorf("failed to commit transaction for loading company profiles: %w", err)
		}
